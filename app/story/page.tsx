...
export default function StoryPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (correct) {
      ...
      setTimeout(() => {
        confetti({ particleCount: 50, angle: 60, spread: 55, origin: { x: 0 }, colors: ['#FF6B9D', '#C44569', '#FFA07A', '#FFD700', '#FF69B4', '#9370DB'] })
        confetti({ particleCount: 50, angle: 120, spread: 55, origin: { x: 1 }, colors: ['#FF6B9D', '#C44569', '#FFA07A', '#FFD700', '#FF69B4', '#9370DB'] })
      }, 250)
    } else {
      ...
      for (let i = 0; i < emojiCount; i++) {
        ...
        newEmojis.push({ id: Date.now() + i, emoji, x: centerX + Math.cos(angle) * distance, y: centerY + Math.sin(angle) * distance })
      }
      setWrongEmojis(newEmojis)
      setTimeout(() => setWrongEmojis([]), 2000)
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-100 via-purple-50 to-blue-100 p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12 max-w-2xl w-full text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-pink-600 mb-4">Amazing Work, Zoe! 🎌✨</h1>
          <div className="text-3xl mb-6"><span className="text-purple-600 font-bold">Score: {score}/10</span></div>
          <div className="text-2xl mb-6 text-gray-700">Time: {formatTime(timeElapsed)}</div>
          <div className="bg-yellow-100 rounded-2xl p-6 mb-6 border-2 border-yellow-300">
            <p className="text-xl md:text-2xl text-gray-800 font-semibold">{selectedJoke}</p>
          </div>
          <div className="flex flex-wrap gap-4 justify-center">
            <button onClick={generateQuestions} className="bg-gradient-to-r from-pink-500 to-purple-500 text-white text-xl font-bold py-4 px-8 rounded-full hover:from-pink-600 hover:to-purple-600 transform hover:scale-105 transition-all shadow-lg">Play Again! 🎮</button>
            <Link href="/" className="bg-slate-200 text-slate-700 text-xl font-bold py-4 px-8 rounded-full hover:bg-slate-300 transform hover:scale-105 transition-all shadow-lg">🏠 Home</Link>
            <Link href="/arithmetic" className="bg-blue-200 text-blue-700 text-xl font-bold py-4 px-8 rounded-full hover:bg-blue-300 transform hover:scale-105 transition-all shadow-lg">Arithmetic 🔢</Link>
            <Link href="/zombie" className="bg-amber-200 text-amber-800 text-xl font-bold py-4 px-8 rounded-full hover:bg-amber-300 transform hover:scale-105 transition-all shadow-lg">Zombie Survival 🧟</Link>
          </div>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-100 via-purple-50 to-blue-100 p-4 relative overflow-hidden">
      <div className="fixed inset-0 pointer-events-none z-50">
        {wrongEmojis.map((emojiData) => (
          <div key={emojiData.id} className="absolute text-6xl animate-emoji-burst" style={{ left: `${emojiData.x}px`, top: `${emojiData.y}px`, transform: 'translate(-50%, -50%)' }}>
            {emojiData.emoji}
          </div>
        ))}
      </div>
      <div className="max-w-4xl mx-auto relative z-10">
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 flex justify-between items-center">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-pink-600">Zoe&apos;s Anime Math Quiz 🎌</h1>
            <p className="text-gray-600 mt-1">Question {currentQuestionIndex + 1} of {questions.length}</p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold text-purple-600">⏱️ {formatTime(timeElapsed)}</div>
            <div className="text-lg text-gray-600 mt-1">Score: {score}</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-12 mb-6">
          <div className="text-center">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6 leading-relaxed">{currentQuestion.question}</h2>
            {showResult ? (
              ...
              <form onSubmit={handleSubmit} className="mt-8">
                <div className="flex flex-col items-center gap-4">
                  <input type="number" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} className="text-4xl text-center w-32 h-20 border-4 border-pink-300 rounded-2xl focus:border-pink-500 focus:outline-none focus:ring-4 focus:ring-pink-200" autoFocus required />
                  <button type="submit" className="bg-gradient-to-r from-pink-500 to-purple-500 text-white text-xl font-bold py-4 px-8 rounded-full hover:from-pink-600 hover:to-purple-600 transform hover:scale-105 transition-all shadow-lg mt-4">Submit Answer ✨</button>
                </div>
              </form>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          <button onClick={generateQuestions} className="bg-gray-200 text-gray-700 text-lg font-semibold py-3 px-6 rounded-full hover:bg-gray-300 transition-all">🔄 Start New Quiz</button>
          <Link href="/" className="bg-slate-200 text-slate-700 text-lg font-semibold py-3 px-6 rounded-full hover:bg-slate-300 transition-all">🏠 Home</Link>
          <Link href="/arithmetic" className="bg-blue-200 text-blue-700 text-lg font-semibold py-3 px-6 rounded-full hover:bg-blue-300 transition-all">🔢 Arithmetic</Link>
          <Link href="/zombie" className="bg-amber-200 text-amber-800 text-lg font-semibold py-3 px-6 rounded-full hover:bg-amber-300 transition-all">🧟 Zombie Survival</Link>
        </div>
      </div>
    </div>
  )
